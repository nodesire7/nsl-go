...
- **未完成 / 待推进**
  - ❌ **审计日志**：管理员操作、敏感操作缺少审计记录（redo.md 1.2, 2.7, 7.1）
  - ❌ **RBAC 权限点**：目前仅 `admin/user` 角色字段，缺少细粒度权限点（`link:create`, `link:delete`, `domain:manage`, `settings:update` 等）（redo.md 4.2, 6.2）
  - ❌ **Meilisearch 写入失败补偿/重试/后台任务**：当前是简单实时写，缺少重试队列和补偿机制（redo.md 2.6）
  - ❌ **结构化日志统一**：仍有 `log.Printf` 混用，需统一使用 `utils` logger（redo.md 2.7）
  - ❌ **集成测试**：缺少 PG/Redis/Meili 的集成测试（建议 testcontainers）（redo.md 6.3）
  - ❌ **CI 质量工具**：`golangci-lint` / `gosec` 未在 CI 中落地（redo.md 6.3）
  - ❌ **Metrics（Prometheus）**：缺少指标收集和暴露（redo.md 6.3）
  - ❌ **Tracing**：缺少分布式追踪（redo.md 3.1）
  - ❌ **聚合统计扩展**：当前仅有基础统计，缺少日/周/月、来源、UA 等维度（redo.md 5.3）
  - ❌ **限流策略优化**：当前是简单计数器，缺少滑动窗口/令牌桶策略（redo.md 2.5）
  - ❌ **代理链路真实 IP 处理**：缺少 X-Forwarded-For / X-Real-IP 处理（redo.md 2.5）
