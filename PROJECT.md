...
├── utils/
│   └── logger.go                 # 日志工具
├── internal/                      # 重写版分层（与 legacy 并行，逐步迁移）
│   ├── app/                       # 启动封装（挂载 v1 + v2）
│   ├── auth/                      # JWT（v2）
│   ├── config/                    # 重写版配置加载（强校验）
│   ├── db/                        # pgxpool + 迁移器骨架
│   ├── httpv2/                    # v2 路由/handler/middleware
│   ├── repo/                      # v2 repo（pgxpool）
│   └── service/                   # v2 service（业务逻辑）
├── web/
│   ├── templates/
│   │   └── index.html            # Web UI模板
│   └── static/
...
### 开发环境
1. 安装Go 1.21+（推荐提交 go.sum：`go mod tidy`）
